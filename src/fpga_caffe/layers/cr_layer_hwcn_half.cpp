...
#pragma HLS ARRAY_PARTITION variable=relu_en complete dim=2
  ...
  short ofm_iters_z = (fc == 1) ? ofm_iters : 1;
  short ofm_iters_i = (fc == 0) ? ofm_iters : 1;
...
#pragma HLS dependence variable=inbuf_relu inter false
            ...
            for (int o = 0; o < ofm_iters_i; ++o) {
              int ofm_idx = (fc == 1) ? z : o;
              if ((n == 0) && (backward == 0)) {
...
#pragma HLS pipeline
                ...
                for (int k = 0; k < OCFACT; ++k) {
                  ...
                  out_idx_b = ((ofm_idx * OCFACT + k) * burstoc + outchannels *
                      group_idx) * ksize * ksize * ic_fact + n * ksize * ksize *
                      wc_fact;
              ...
              for (int b = 0; b < burstoc; ++b) {
                for (int k = 0; k < OCFACT; ++k) {
                  ...
                  w_idx_b = (((y * xdim_out + x) * numgroups + group_idx) *
                      outchannels + (ofm_idx * OCFACT + k) * burstoc + b) *
                      img_fact;
                  w_size_b = img_fact;
                  w_idx_f = ((ofm_idx * OCFACT + k) * burstoc + b +
                    outchannels * group_idx) * ksize * ksize * ic_fact + n *
                    ksize * ksize * wc_fact;
                  ...
                  bool write_en = ((ofm_idx * OCFACT + k) * burstoc + b <
                      outchannels) && ((fc == 0) || (mode) || ((fc == 1) &&
                      (x == 0) && (y == 0)));
                  if (write_en)
...
#pragma HLS DEPENDENCE variable wUpdate inter false
                  ...
                  short in_idx = (filt_off * burst_fact + w_off) * img_fact
                    + img_off;
                  short out_idx_f = b * img_fact + img_off;
                  short out_idx_b = b * ksize * ksize * wc_fact +
                    filt_off * wc_fact + (w_off >> 2);
              ...
              if (relu && (backward == 0) && (n == rpo - 1)) {
                relu_fw(outbuf, outbuf_relu, burstoc * img_fact);
              }
