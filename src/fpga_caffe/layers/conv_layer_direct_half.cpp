...
void input_stage(chalf16 inbuf[256 * 16], unsigned short ksize,
...
void conv_layer_direct_half(chalf16 *input, chalf16 *weights, chalf *bias,
...
#pragma HLS ARRAY_PARTITION variable=ot_s1 complete dim=1
  ...
  for (int n = 0; n < rpo; ++n) {
    ...
    memcpy(inbuf, input + in_off, sizeof(chalf16) * ((burstchannels * ydim * 
            xtile_pad * 2) >> 4)); 
...
#pragma HLS pipeline
          ...
          for (int k = 0; k < OCFACT; ++k) {
            out_offset = image_idx * numgroups * outchannels * ydim * fact +
              ...
              burstydim) * fact;
            memcpy(outbuf[k], output + out_offset, sizeof(chalf16) * fact *
                burstydim);
          }
        }
...
#pragma HLS pipeline        
        ...
        for (int k = 0; k < OCFACT; ++k) {
          ...
          if (o * OCFACT + k < outchannels) {
            memcpy(output + out_offset, outbuf[k], sizeof(chalf16) * fact *
                burstydim);
          }
        }
      }   
