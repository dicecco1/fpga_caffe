...
void input_stage(cpfp16 inbuf[256 * 16], unsigned short ksize,
...
void conv_layer_direct_cpfp(cpfp16 *input, cpfp16 *weights, cpfp *bias,
...
#pragma HLS ARRAY_PARTITION variable=ot_s1 complete dim=1
  ...
  for (int n = 0; n < rpo; ++n) {
    ...
    memcpy(inbuf, input + in_off, sizeof(cpfp16) * ((burstchannels * ydim * 
            xtile_pad * 2) >> 4)); 
...
#pragma HLS pipeline
          ...
          for (int k = 0; k < OCFACT; ++k) {
            out_offset = image_idx * numgroups * outchannels * ydim * fact +
              ...
              burstydim) * fact;
            memcpy(outbuf[k], output + out_offset, sizeof(cpfp16) * fact *
                burstydim);
          }
        }
...
#pragma HLS pipeline        
        ...
        for (int k = 0; k < OCFACT; ++k) {
          ...
          if (o * OCFACT + k < outchannels) {
            memcpy(output + out_offset, outbuf[k], sizeof(cpfp16) * fact *
                burstydim);
          }
        }
      }   
