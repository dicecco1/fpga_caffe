...
#pragma HLS ARRAY_PARTITION variable=otb complete dim=2
  ...
  for (int n = 0; n < rpo; ++n) {
    ...
    for (int image_off = 0; image_off < numimages; ++image_off) {
      ...
      if (relu && backward_flag) {
        memcpy(inbuf_relu, track_relu + in_off, sizeof(char16) *
          burstchannels * ydim * fact);
        RELU_BW: for (int i = 0; i < burstchannels * ydim * fact; ++i) {
        ...
        for (int offy = 0; offy < rpofm; ++offy) {
          ...
          for (int k = 0; k < OCFACT; ++k) {
            if (backward_flag) {
              ...
              if (relu) {
                RELU_FW: for (int i = 0; i < fact * burstydim; ++i) {
                #pragma HLS pipeline
                  chalf16 val;
