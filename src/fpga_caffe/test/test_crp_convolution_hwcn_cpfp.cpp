...
class CRPConvolutionHWCNCPFPTest : public OCLDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  CRPConvolutionHWCNCPFPTest()
  ...
  virtual ~CRPConvolutionHWCNCPFPTest() {}
...
TYPED_TEST_CASE(CRPConvolutionHWCNCPFPTest, TestOCLDtypesAndDevices);

TYPED_TEST(CRPConvolutionHWCNCPFPTest, TestConvReLU1x1F_CPFP) {
  this->ocl.Setup();
...
TYPED_TEST(CRPConvolutionHWCNCPFPTest, TestConvReLU1x1B_CPFP) {
  this->ocl.Setup();
  std::vector<kernel_params> params = this->params;
  std::vector<cl_event> events;
