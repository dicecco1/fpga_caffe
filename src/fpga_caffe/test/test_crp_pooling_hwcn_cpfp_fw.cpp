...
class CRPFWPoolingHWCNCPFPTest : public OCLDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  CRPFWPoolingHWCNCPFPTest()
    : ocl("crp_layer_hwcn_cpfp_fw.xclbin", "crp_layer_hwcn_cpfp_fw") 
  {}
  virtual void SetUp() {
    params.resize(1);
  ...
  virtual ~CRPFWPoolingHWCNCPFPTest() {}
...
TYPED_TEST_CASE(CRPFWPoolingHWCNCPFPTest, TestOCLDtypesAndDevices);

TYPED_TEST(CRPFWPoolingHWCNCPFPTest, TestCRPFW2x2F_Pool_CPFP) {
  this->ocl.Setup();
...
TYPED_TEST(CRPFWPoolingHWCNCPFPTest, TestCRPFW3x3F_Pool_CPFP) {
  this->ocl.Setup();
  std::vector<kernel_params> params = this->params;
  std::vector<cl_event> events;
