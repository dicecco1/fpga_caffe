...
class WCRPPoolingHWCNCPFPTest : public OCLDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  WCRPPoolingHWCNCPFPTest()
  ...
  virtual ~WCRPPoolingHWCNCPFPTest() {}
...
TYPED_TEST_CASE(WCRPPoolingHWCNCPFPTest, TestOCLDtypesAndDevices);

TYPED_TEST(WCRPPoolingHWCNCPFPTest, TestWCRP2x2F_Pool_CPFP) {
  this->ocl.Setup();
...
TYPED_TEST(WCRPPoolingHWCNCPFPTest, TestWCRP3x3F_Pool_CPFP) {
  this->ocl.Setup();
  std::vector<kernel_params> params = this->params;
  std::vector<cl_event> events;
