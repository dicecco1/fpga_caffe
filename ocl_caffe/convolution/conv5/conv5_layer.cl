...
void conv5_layer(__global float *a, __global float *b, __global float *output)
{
  ...
  __local float input[NUM_DATA_ROWS * NUM_DATA_COLS];// __attribute__((xcl_array_partition(cyclic, NUM_DATA_COLS , 1)));
  __local float bufout[NUM_OUT_ROWS * NUM_OUT_COLS];// __attribute__((xcl_array_partition(block, NUM_OUT_COLS, 1))); 
...
//  int k = get_global_id(1);
  ...
  int idx_y[NUM_OUT_ROWS][NUM_MASK_ROWS] __attribute__((xcl_array_partition(complete, 2)));
  ...
  for (int g = 0; g < GROUPS; ++g) {
    ...
    for (int i = 0; i < BURST; ++i) {
      ...
      data_idx_t = 0;//NUM_DATA_ROWS * NUM_DATA_COLS;
