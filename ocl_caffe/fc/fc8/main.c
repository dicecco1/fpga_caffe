...
//  gemm(a1, b1, sw_results2);
  ...
  //
  kernel = clCreateKernel(program, "fc8_layer", &err);
  ...
  cl_event readevent;
  err = clEnqueueReadBuffer(commands, output, CL_FALSE, 0, sizeof(float) * OUTPUT_SIZE1, results1, 0, NULL, &readevent );  
  if (err != CL_SUCCESS)
  {
